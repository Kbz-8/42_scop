...
namespace Scop
{
	...
	{
		public:
			NativeScript(std::function<void(NonOwningPtr<class Actor>)> on_init, std::function<void(NonOwningPtr<class Actor>, class Inputs&, float)> on_update, std::function<void(NonOwningPtr<class Actor>)> on_quit)
			...
			inline void OnUpdate(NonOwningPtr<class Actor> actor, class Inputs& input, float delta) override { if(f_on_update) f_on_update(actor, input, delta); }
		...
		private:
			...
			std::function<void(NonOwningPtr<class Actor>, class Inputs&, float)> f_on_update;
